<template>
  <div class="category">
    ...
    <div class="list-box">
      <div class="left">
        <ul>
          <li v-for="(item, index) in list" :key="item.category_id">
            <a :class="{ active: index === activeIndex }" href="javascript:" @click="activeIndex = index">{{
                item.name
              }}</a>
          </li>
        </ul>
      ...
      <div class="right">
        <div v-for="item in list[activeIndex]?.children"
...
export default {
  ...
  methods: {
    async getCategoryList () {
      const { data: { list } } = await getCategoryListApi()
      this.list = list
    }
  }
}
